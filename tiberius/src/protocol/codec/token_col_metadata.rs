use super::{read_varchar, BytesData, Decode, TypeInfo};
...
impl<'a> Decode<'a, BytesMut> for TokenColMetaData {
        ...
        if column_count > 0 && column_count < 0xffff {
            ...
            for _ in 0..column_count {
                ...
                let meta = MetaDataColumn {
                    base: BaseMetaDataColumn::decode(src)?,
                    col_name: read_varchar(src, len)?,
                };
                columns.push(meta);
            }
        }
...
impl<'a> Decode<'a, BytesMut> for BaseMetaDataColumn {
    fn decode(src: &mut BytesMut) -> crate::Result<Self>
    where
        Self: Sized,
    {
